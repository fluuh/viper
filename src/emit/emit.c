...
vn_reg *vn_reg_create(vn_bfunc *fn, vp_type type)
{
	if(fn->cap_regs <= fn->num_regs) {
		fn->cap_regs *= 2;
		fn->regs = vu_realloc(fn->regs,
		                      sizeof(*fn->regs) * fn->cap_regs);
	}
...
static int emit_bytes(vn_bfunc *fn, u8* src, size_t size)
{
	if(fn->cap_code <= fn->size_code + size) {
		fn->cap_code *= 2;
		fn->code = vu_realloc(fn->code,
		                      sizeof(*fn->code) * fn->cap_code);
...
static int emit_bin(vn_bfunc *fn, u8 op, 
                    vn_reg *dst, vn_reg *op0, vn_reg* op1)
{
	vp_type type = dst->type;
...
{
	return emit_byte(fn, OP_NOP);
}

int vn_emit_end(vn_bfunc *fn)
{
	return emit_byte(fn, OP_END);
}
...
static int emit_call(vn_bfunc *fn, vn_reg *dst, 
                     i32 id, u8 num_args, vn_reg **args) 
{
