...
struct as_sym {
	...
	struct as_sym *prev;
	const char *name;
	enum { sym_data, sym_extern, sym_func } type;
	i32 index;
};
...
static as_sym *new_symbol(assembler *as, const char *name, int type, i32 index)
{
...
static int parse_extern(assembler *as)
{
	...
	ret = tok->vtype;
	vn_import *imp =
	    vn_bimport(as->builder, name->str, ret, args, num_args);
...
static int parse_call(assembler *as, vn_bfunc *fn, vn_reg **regs, i32 reg)
{
	...
	if (sym->type == sym_extern) {
		...
		vn_emit_call(fn, to, as->builder->funcs[sym->index], args,
		             argc);
		return 0;
	}
	return 1;
...
static int parse_setter(assembler *as, vn_bfunc *fn, vn_reg **regs, token *_reg)
{
	u8 reg = _reg->num;
	token *tok = get_token(as);
	switch (tok->type) {
