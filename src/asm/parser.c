...
static struct asm_inst *asm_emit(struct asm_code *code, int op, struct asm_operand *operands, int len)
{
 struct asm_inst *inst = vi_malloc(sizeof(*inst) + sizeof(*inst->args) * len);
 inst->op = op;
 inst->len = len;
...
static void asm_code_free(struct asm_code *code)
{
 ...
 while(inst != NULL) {
  for(int i = 0; i < inst->len; i++) {
   if(inst->args[i].ty == asm_name || inst->args[i].ty == asm_label) {
